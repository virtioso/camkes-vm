...
/*- if config_devices is not none -*/
    /*- for device in config_devices -*/
        ...
        /*- else -*/
            /*- set owner = 0 -*/
        /*- endif -*/
        /*- do devices.append((device['name'].strip('"'), device['bus'], device['dev'], device['fun'], owner)) -*/
    /*- endfor -*/
/*- endif -*/
...
int physical_pci_devices_get_device(int num, uint8_t *bus, uint8_t *dev, uint8_t *fun, int *owner)
{
    ...
    /*- else -*/
        switch (num) {
            /*- for name, bus, dev, fun, owner in devices -*/
            case /*? loop.index0 ?*/:
                *bus = /*? bus ?*/;
                *dev = /*? dev ?*/;
                *fun = /*? fun ?*/;
...
int physical_pci_devices_get_owner(uint8_t bus, uint8_t dev, uint8_t fun)
{
    ...
    /*- else -*/
            /*- for name, device_bus, device_dev, device_fun, owner in devices -*/
                if (bus == /*? device_bus ?*/ &&
                    dev == /*? device_dev ?*/ &&
                    fun == /*? device_fun ?*/) {
                    return /*? owner ?*/;
